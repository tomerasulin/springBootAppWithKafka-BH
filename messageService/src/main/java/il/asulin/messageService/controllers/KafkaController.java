...
public class KafkaController {
	...
	public ResponseEntity<?> getJsonObject(@RequestBody String jsonString) {
		try {
			...
			this.service.sendToProducer(convertedString);//Send to the Producer the XML format
			JSONObject json = messageObserver.waitForMessage();//Waiting for response from the Consumer - getting the JSON object
